...
function cws_setup() {
	 ...
	 * If you're building a theme based on cws, use a find and replace
	 * to change 'cws' to the name of your theme in all the template files
	 */
	load_theme_textdomain( 'cws', get_template_directory() . '/languages' );
	 ...
	 * Image sizes
	 */
	add_image_size( 'preview', 440, 440, true );

}
endif; // cws_setup
add_action( 'after_setup_theme', 'cws_setup' );
...
--------------------------------------------------*/
function cws_scripts() {
	$postfix = ( defined( 'WP_DEBUG' ) && true === WP_DEBUG ) ? '' : '.min';

	wp_enqueue_style( 'cws-style', get_template_directory_uri() . "/style{$postfix}.css" );
...
add_action( 'wp_enqueue_scripts', 'cws_scripts' );
